...
const ChatWindow: React.FC<ChatWindowProps> = ({ isOpen, onClose, workflowId }) => {
  ...
  useEffect(() => {
    if (isOpen && workflowId && !chatId) {
      createChatMutation.mutate(workflowId)
    }
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const stream = api.sendMessage(workflowId, chatId, userMessage.content)
      ...
      setMessages(prev => 
        prev.map(msg => 
          msg.id === assistantMessage.id 
            ? { ...msg, content: 'Error: Failed to get response from workflow. Please check if your workflow is properly built and try again.' }
            : msg
        )
      )
    } finally {
            ...
            <div className="flex items-center justify-center h-32">
              <div className="text-center">
                ...
                <div className="text-gray-400 text-sm mt-1">
                  Your workflow is ready to process queries through the nodes you've created
                </div>
              </div>
            </div>
          ) : (
